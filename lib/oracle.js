...
Oracle.prototype.create = function (model, data, callback) {
  ...
  this.query(sql, [outParam], function (err, info) {
    if (err) return callback(err);
...
Oracle.prototype.toFields = function (model, data, forCreate) {
  ...
  if (forCreate) {
    ...
    Object.keys(data).forEach(function (key) {
      if (props[key]) {
        fields.push(self.columnEscaped(model, key) + '= '
          + this.toDatabase(props[key], data[key]));
      }
    }.bind(this));
    return fields.join(',');
  }
...
Oracle.prototype.toDatabase = function (prop, val) {
  ...
  if (val.constructor.name === 'Object') {
    ...
    val = val[operator];
    if (operator === 'between') {
      return this.toDatabase(prop, val[0]) + ' AND '
