 ...
 *
 *
 * @param {object} oracle Oracle node.js binding
...
Oracle.prototype.connect = function (callback) {
  ...
  this.oracle.createConnectionPool(this.settings, function (err, pool) {
    if (!err) {
      self.pool = pool;
 ...
 */
Oracle.prototype.executeSQL = function (sql, params, callback) {
  var self = this;
  var time = Date.now();
  var log = self.log;
  ...
  self.pool.getConnection(function (err, connection) {
    ...
    if (self.settings.debug) {
      self.debug('Connection acquired: ', self.pool.getInfo());
    }
    connection.execute(sql, params, function (err, data) {
      ...
      connection.close();
      if (self.settings.debug) {
        self.debug('Connection released: ', self.pool.getInfo());
      }
 ...
 *
 */
Oracle.prototype.executeSQLSync = function (sql, params) {
  var self = this;
  if (self.settings.debug) {
    if (params && params.length > 0) {
      self.debug('SQL: %s \nParameters: %j', sql, params);
  ...
  if (!self.pool) {
    self.pool = oracle.createConnectionPoolSync(self.settings);
    if (self.settings.debug) {
      self.debug('Connection acquired: ', self.pool.getInfo());
    }
  }
  var connection = self.pool.getConnectionSync();
  try {
    var data = connection.executeSync(sql, params);
    if (self.settings.debug && data) {
      self.debug("%j", data);
    }
    connection.close();
 ...
 * @returns {*} The results
 */
Oracle.prototype.querySync = function (sql, params) {
  params = params || [];

  return this.executeSQLSync(sql, params);
};
 ...
 *
 */
Oracle.prototype.destroyAll = function destroyAll(model, where, callback) {
...
Oracle.prototype.create = function (model, data, callback) {
  ...
  sql += ' RETURNING ' + this.idColumnEscaped(model) + ' into :' + (fields.params.length + 1);

  var type = this._models[model].properties[this.idName(model)].type;
  var outParam = null;
  if (type === String) {
    outParam = new oracle.OutParam(oracle.OCCISTRING);
  } else {
    outParam = new oracle.OutParam();
  }

  var params = fields.params;
  params.push(outParam);
...
Oracle.prototype.save = function (model, data, callback) {
  ...
  var sql = 'UPDATE ' + this.tableEscaped(model) + ' SET ' + fields.sql;
...
Oracle.prototype.toFields = function (model, data, forCreate) {
  ...
  if (forCreate) {
    Object.keys(data).forEach(function (key) {
      if (props[key]) {
        if (!self.id(model, key)) {
          if (props[key].type === String || props[key].type === Buffer) {
    ...
    result.sql = '(' + columns.join(',') + ') VALUES (' + fields.join(',') + ')';
    ...
    Object.keys(data).forEach(function (key) {
      if (props[key]) {
        if (props[key].type === String || props[key].type === Buffer) {
          columns.push(self.columnEscaped(model, key) + '= ' + (':' + (++index)));
          params.push(this.toDatabase(props[key], data[key], true));
        } else {
          columns.push(self.columnEscaped(model, key) + '= ' + this.toDatabase(props[key], data[key]));
        }
      }
    }.bind(this));
    result.sql = columns.join(',');
    result.params = params;
    return result;
  }
};
 ...
 * @param {Boolean} raw
 */
Oracle.prototype.toDatabase = function (prop, val, raw) {
...
Oracle.prototype.fromDatabase = function (model, data) {
  ...
  for (var p in props) {
    ...
    if (prop && prop.type && prop.type.name === 'Boolean') {
      json[p] = (val === 'Y' || val === 'y' || val === 'T' || val === 't'
        || val === '1');
    } else {
      json[p] = val;
    }
 ...
 * @returns {String}
 */
Oracle.prototype.getColumns = function (model, props) {
...
function getPropertiesToModify(model, actualFields) {
  ...
  function nullabilityChanged(propName, oldSettings) {
    ...
    var changed = false;
    if (oldSettings.nullable === 'Y' && (newSettings.allowNull === false
      || newSettings.null === false)) {
      changed = true;
    }
    if (oldSettings.nullable === 'N' && !(newSettings.allowNull === false
      || newSettings.null === false)) {
      changed = true;
    }
    return changed;
...
Oracle.prototype.createTable = function (model, cb) {
  ...
  var idGenerated = self.idNames(model).length > 1 ? false : self._models[model].properties[self.idName(model)].generated;
